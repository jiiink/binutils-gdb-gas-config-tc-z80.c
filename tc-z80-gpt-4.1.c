...
static void setup_march(const char *name, int *ok, int *err, int *mode)
{
    ...
    if (name == NULL || ok == NULL || err == NULL || mode == NULL) {
        as_fatal(_("Invalid argument to setup_march"));
        return;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(match_cpu_table); ++i) {
        if (strncasecmp(p, match_cpu_table[i].name, len) == 0 &&
    ...
    while (*p) {
        ...
        for (i = 0; i < ARRAY_SIZE(match_ext_table); ++i) {
            if (strncasecmp(p, match_ext_table[i].name, len) == 0 &&
...
static int setup_instruction(const char *inst, int *add, int *sub) {
    ...
    if (strcmp(inst, "idx-reg-halves") == 0)
        n = INS_IDX_HALF;
    else if (strcmp(inst, "sli") == 0)
        n = INS_SLI;
    else if (strcmp(inst, "op-ii-ld") == 0)
        n = INS_ROT_II_LD;
    else if (strcmp(inst, "in-f-c") == 0)
        n = INS_IN_F_C;
    else if (strcmp(inst, "out-c-0") == 0)
        n = INS_OUT_C_0;
    else
        return 0;
...
static int setup_instruction_list(const char *list, int *add, int *sub)
{
    ...
    while (*b != '\0') {
        ...
        int sz = (e == NULL) ? (int)strlen(b) : (int)(e - b);
...
void md_begin(void) {
    expressionS nul, reg;
    char *saved_input;
    unsigned int i, j, bit_combo, name_len;
    char buf[BUFLEN];
    ...
    for (i = 0; i < ARRAY_SIZE(regtable); ++i) {
        ...
        name_len = strlen(regtable[i].name);
        if (name_len + 1 >= BUFLEN)
            continue;
        for (bit_combo = 1U << name_len; bit_combo; --bit_combo) {
            for (j = 0; j < name_len; ++j) {
                buf[j] = (bit_combo & (1U << j)) ? TOUPPER(regtable[i].name[j]) : regtable[i].name[j];
            }
            buf[name_len] = '\0';
...
void z80_md_finish(void)
{
    ...
    {
        /* Appropriate error handling, e.g., logging or aborting */
        abort();
    }
}
...
int z80_start_line_hook(void)
{
    ...
    for (p = input_line_pointer; *p && *p != '\n'; ++p) {
        ...
        case '\'':
            if (p[1] && p[1] != '\'' && p[2] == '\'') {
                int ret = snprintf(buf, sizeof(buf), "%3d", (unsigned char)p[1]);
                if (ret != 3) {
                    as_bad(_("snprintf error in character constant"));
                    ignore_rest_of_line();
                    return 1;
                }
                *p++ = buf[0];
    ...
    if (is_name_beginner(*input_line_pointer)) {
        ...
        if (len && (len <= 2 || !ISALPHA(rest[len]))) {
            if (line_start > input_line_pointer && line_start[-1] == '\n') {
                bump_line_counters();
                LISTING_NEWLINE();
            }
...
md_atof(int type, char *litP, int *sizeP)
{
    ...
    } else if ((type == 'd' || type == 'D' || type == 'r' || type == 'R') && str_to_double) {
...
parse_exp_not_indexed(const char *s, expressionS *op)
{
    ...
    if (indir && (ins_ok & INS_GBZ80)) {
        ...
        if (!strncasecmp(p, "hl", 2)) {
            const char *q = skip_space(p + 2);
            if ((*skip_space(q + 1) == ')') && (*q == '+' || *q == '-')) {
                op->X_op = O_md1;
                op->X_add_symbol = NULL;
                op->X_add_number = (*q == '+') ? REG_HL : -REG_HL;
                input_line_pointer = (char *)skip_space(q + 1) + 1;
...
static int unify_indexed(expressionS *op) {
    ...
    if (val_expr == NULL || O_register != val_expr->X_op)
        return 0;
...
static const char *parse_cc(const char *s, char *op) {
    ...
    while (i < BUFLEN && ISALPHA(s[i])) {
        buf[i] = TOLOWER(s[i]);
        i++;
    }

    if (i > 0 && i < BUFLEN && (s[i] == '\0' || s[i] == ',')) {
        buf[i] = '\0';
        cc_p = bsearch(&key, cc_tab, ARRAY_SIZE(cc_tab), sizeof(cc_tab[0]), key_cmp);
    }
...
static void emit_data_val(expressionS *val, int size)
{
    ...
    p = frag_more(size);
    if (!p)
        as_fatal(_("frag_more failed for size %d"), size);
...
static void emit_byte(expressionS *val, bfd_reloc_code_real_type r_type)
{
    ...
    if (val == NULL) {
        as_bad(_("NULL pointer for 'val' in emit_byte"));
        return;
    }
    ...
    p = frag_more(1);
    if (!p) {
        as_bad(_("frag_more(1) failed in emit_byte"));
        return;
    }
    *p = (char)val->X_add_number;
    ...
    fix_new_exp(frag_now, (int)(p - frag_now->fr_literal), 1, val, r_type == BFD_RELOC_8_PCREL, r_type);
}

static void emit_word(expressionS *val)
{
  int size = (inst_mode & INST_MODE_IL) ? 3 : 2;
  emit_data_val(val, size);
}
    ...
    if (arg_m.X_op == O_md1) {
        if (*p == ',') {
            ...
            if (arg_r.X_md == 0 &&
                ...
                opcode += (char)(arg_r.X_add_number - 6);
            } else {
                ill_op();
                return p;
            }
...
emit_sub(char prefix, char opcode, const char *args)
{
    ...
    if (!args) {
        error(_("missing instruction arguments"));
        return args;
    }
    ...
    p = parse_exp(p, &arg_s);
    if (!p) {
        error(_("bad instruction argument"));
        return args;
    }
...
emit_incdec(char prefix, char opcode, const char *args)
{
    ...
    if (!operand.X_md && operand.X_op == O_register && (R_ARITH & rnum)) {
        char *q = NULL;
        size_t frag_len = (rnum & R_INDEX) ? 2 : 1;
        q = frag_more(frag_len);
        if (!q) {
            ill_op();
            return p;
        }
...
emit_jp(char prefix, char opcode, const char *args)
{
    ...
    if (addr.X_md) {
        ...
        if ((addr.X_op == O_register) && (rnum == REG_C) && (ins_ok & INS_Z80N)) {
            q = frag_more(2);
            if (!q) ill_op();
            *q++ = 0xED;
            *q = 0x98;
...
emit_im(char prefix, char opcode, const char *args)
{
    ...
    if (add_num == 0 || add_num == 1 || add_num == 3) {
        q = frag_more(2);
        if (q == NULL) {
            ill_op();
            return NULL;
...
emit_pop(char prefix ATTRIBUTE_UNUSED, char opcode, const char *args)
{
    ...
    char *q = (rnum & R_INDEX) ? frag_more(2) : frag_more(1);
...
emit_push(char prefix, char opcode, const char *args)
{
    ...
    fix_new_exp(frag_now, (int)(q - frag_now->fr_literal), 2, &arg, false, BFD_RELOC_Z80_16_BE);
...
{
    expressionS op;
    const char *p = parse_exp_not_indexed(args, &op);
    p = skip_space(p);
    if (*p != ',') {
        error(_("bad instruction syntax"));
        return p;
    ...
    if (op.X_op == O_register) {
        unsigned reg_val = op.X_add_number | (op.X_md ? 0x8000 : 0);
        switch (reg_val) {
            case REG_AF:
                if (TOLOWER(*p) == 'a' && TOLOWER(*(p + 1)) == 'f') {
                    p += 2;
                    if (*p == '`') p++;
                    opcode = 0x08;
                }
...
emit_out0(char prefix ATTRIBUTE_UNUSED, char opcode ATTRIBUTE_UNUSED, const char *args)
{
    ...
    int is_valid_port =
        (port.X_md != 0) &&
        (port.X_op == O_register || port.X_op == O_md1);
    int is_valid_reg =
        (reg.X_md == 0) &&
        (reg.X_op == O_register) &&
        (reg.X_add_number <= 7);

    if (is_valid_port && is_valid_reg) {
        char *q = frag_more(2);
        if (q == NULL) {
            error(_("memory allocation failed in frag_more"));
            return p;
        }
        q[0] = 0xED;
        q[1] = 0x01 | (reg.X_add_number << 3);
        emit_byte(&port, BFD_RELOC_8);
    } else {
        ill_op();
    }
...
emit_rst(char prefix ATTRIBUTE_UNUSED, char opcode, const char *args)
{
    ...
    char *q = frag_more(1);
    if (q == NULL) {
        error("Memory allocation failed in frag_more");
        return p;
    }
...
static void emit_ld_m_n(expressionS *dst, expressionS *src)
{
    ...
    if (!q)
    {
        /* Handle frag_more failure if necessary */
        return;
    }
...
static void emit_ld_m_rr(expressionS *dst, expressionS *src) {
    ...
    switch (dst->X_op) {
        ...
        case O_register:
            ...
            q = frag_more((prefix ? 2 : 1) + ((prefix == 0xFD || prefix == 0xDD) ? 1 : 0));
            *q++ = prefix;
            *q++ = opcode;
...
static void emit_ld_r_n(expressionS *dst, expressionS *src) {
    ...
    if (prefix) {
        ...
        if ((ins_ok & (INS_EZ80 | INS_R800 | INS_Z80N)) == 0) {
            check_mach(INS_IDX_HALF);
        }
    }
...
static void emit_ld_r_r(expressionS *dst, expressionS *src)
{
    ...
    switch (dst->X_add_number) {
        ...
        case REG_HL:
            ...
            if (src->X_add_number != REG_I) ill_op();
            if (cpu_mode < 1) error(_("ADL mode instruction"));
            prefix = 0xED;
            opcode = 0xD7;
            break;
        case REG_I:
            ...
            break;
        case REG_MB:
            if (!(ins_ok & INS_EZ80) || (src->X_add_number != REG_A)) ill_op();
            if (cpu_mode < 1) error(_("ADL mode instruction"));
            prefix = 0xED;
            opcode = 0x6D;
            break;
...
static void emit_ld_rr_m(expressionS *dst, expressionS *src)
{
    ...
    switch (src->X_op)
    {
        ...
        case O_register:
            ...
            q = frag_more(2);
            *q++ = prefix ? prefix : 0xED;
            *q = opcode;

            if (prefix && prefix != 0xED)
            {
                src_offset = *src;
                src_offset.X_op = O_symbol;
                src_offset.X_add_number = 0;
...
static void emit_ld_rr_nn(expressionS *dst, expressionS *src) {
    ...
    q = frag_more(prefix ? 2 : 1);
    if (!q) {
        /* Handle memory allocation failure or error in frag_more */
        ill_op();
        return;
    }
...
emit_lddldi(char prefix, char opcode, const char *args)
{
    ...
    if (dst.X_md != 0 && dst.X_add_number == REG_HL && src.X_md == 0 && src.X_add_number == REG_A) {
        /* LDx (HL),A: opcode already set */
    } else if (dst.X_md == 0 && dst.X_add_number == REG_A && src.X_md != 0 && src.X_add_number == REG_HL) {
...
emit_ldh(char prefix ATTRIBUTE_UNUSED, char opcode ATTRIBUTE_UNUSED, const char *args)
{
    ...
    if (dst.X_md == 0 && dst.X_op == O_register && dst.X_add_number == REG_A && src.X_md != 0 && src.X_op != O_md1) {
        if (src.X_op == O_register) {
    ...
    } else if (dst.X_md != 0 && dst.X_op != O_md1 && src.X_md == 0 && src.X_op == O_register && src.X_add_number == REG_A) {
        if (dst.X_op == O_register) {
...
emit_mlt(char prefix, char opcode, const char *args)
{
    ...
    if (ins_ok & INS_Z80N) {
        ...
        *q++ = prefix;
        *q = (char)(opcode | ((arg.X_add_number & 3) << 4));
    }
...
static void assume(int arg ATTRIBUTE_UNUSED)
{
    ...
    if (!name || strncasecmp(name, "ADL", 3) != 0)
    {
        ill_op();
        return;
    }
...
static int assemble_suffix(const char **suffix) {
    ...
    *suffix = p;
    ...
    switch (index) {
        case 0: i = cpu_mode ? 0x5B : 0x52; break;
        case 1: i = cpu_mode ? 0x49 : 0x40; break;
        case 2: i = cpu_mode ? 0x5B : 0x49; break;
        case 3: i = 0x5B; break;
        case 4: i = 0x49; break;
        case 5: i = cpu_mode ? 0x52 : 0x40; break;
        case 6: i = 0x52; break;
        case 7: i = 0x40; break;
        default: return 0;
    }
    unsigned char *frag = frag_more(1);
    if (!frag)
        return 0;
    *frag = (unsigned char)i;
...
static void set_inss(int inss)
{
    ...
    int filtered_ins = ins_ok & INS_MARCH_MASK;
    int updated_ins = filtered_ins | inss;

    if (ins_ok != updated_ins)
        cpu_mode = 0;

    ins_ok = updated_ins;
}

static void ignore(int arg ATTRIBUTE_UNUSED) {
    ignore_rest_of_line();
}
...
void md_assemble(char *str)
{
    ...
    i = 0;
    while (i < BUFLEN && (ISALPHA(*p) || ISDIGIT(*p))) {
        buf[i++] = TOLOWER(*p++);
    }
...
static int signed_overflow(signed long value, unsigned int bitsize) {
    if (bitsize == 0 || bitsize >= sizeof(signed long) * 8) {
        return 1;
    }
    signed long max = (1L << (bitsize - 1)) - 1;
    signed long min = - (1L << (bitsize - 1));
...
void md_apply_fix(fixS *fixP, valueT *valP, segT seg) {
    ...
    if (!fixP || !fixP->fx_frag || !valP) {
        abort();
        return;
    }

    p_lit = fixP->fx_where + fixP->fx_frag->fr_literal;

    if (fixP->fx_addsy == NULL) {
        fixP->fx_done = 1;
    } else if (fixP->fx_pcrel) {
        s = S_GET_SEGMENT(fixP->fx_addsy);
...
tc_gen_reloc(asection *seg ATTRIBUTE_UNUSED, fixS *fixp)
{
    ...
    if (reloc == NULL) {
        as_bad(_("Failed to allocate memory for reloc"));
        return NULL;
    }

    reloc->sym_ptr_ptr = notes_alloc(sizeof(asymbol *));
    if (reloc->sym_ptr_ptr == NULL) {
        as_bad(_("Failed to allocate memory for sym_ptr_ptr"));
        return NULL;
    }
...
int z80_tc_label_is_local(const char *name) {
    ...
    const char *p = local_label_prefix;
    const char *n = name;
...
static int str_to_broken_float(bool *signP, uint64_t *mantissaP, int *expP)
{
    ...
    if (!signP || !mantissaP || !expP || !p) return 0;
...
str_to_zeda32(char *litP, int *sizeP)
{
    ...
    *sizeP = 4;
    ...
    for (i = 0; i < 3; ++i) {
        litP[i] = (char)(mantissa >> (i * 8));
    }
    litP[3] = (char)(0x80 + exponent);
...
static const char *str_to_ieee754_s(char *litP, int *sizeP) {
    if (!litP || !sizeP) {
        return NULL;
    }
...
str_to_ieee754_d(const char *litP, int *sizeP)
{
    ...
    return ieee_md_atof('d', (char *)litP, sizeP, false);
}
