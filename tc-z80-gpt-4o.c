...
static void setup_march(const char *name, int *ok, int *err, int *mode) {
    ...
    for (i = 0; i < ARRAY_SIZE(match_cpu_table); ++i) {
        if (strncasecmp(name, match_cpu_table[i].name, len) == 0 && strlen(match_cpu_table[i].name) == len) {
    ...
    while (name[len] != '\0') {
        ...
        for (i = 0; i < ARRAY_SIZE(match_ext_table); ++i) {
            if (strncasecmp(name, match_ext_table[i].name, len) == 0 && strlen(match_ext_table[i].name) == len) {
                if (name[-1] == '+') {
...
static int setup_instruction(const char *inst, int *add, int *sub) {
    ...
    for (size_t i = 0; i < sizeof(instruction_map) / sizeof(instruction_map[0]); ++i) {
        if (strcmp(inst, instruction_map[i].name) == 0) {
            *add |= instruction_map[i].value;
            *sub &= ~instruction_map[i].value;
            return 1;
        }
    }
    return 0;
...
static str_to_float_t get_str_to_float(const char *arg) {
    if (!strcasecmp(arg, "zeda32")) return str_to_zeda32;
    if (!strcasecmp(arg, "math48")) return str_to_float48;
    if (!strcasecmp(arg, "half")) return str_to_ieee754_h;
    if (!strcasecmp(arg, "single")) return str_to_ieee754_s;
    if (!strcasecmp(arg, "double")) return str_to_ieee754_d;

    if (!strcasecmp(arg, "ieee754"))
...
static int setup_instruction_list(const char *list, int *add, int *sub) {
    ...
    while (*b != '\0') {
        ...
        int sz = (e == NULL) ? strlen(b) : (e - b);

        if (sz == 0 || sz >= BUF_SIZE) {
            fprintf(stderr, "invalid INST in command line: %s\n", b);
            return 0;
        }
...
int md_parse_option(int c, const char *arg) {
  switch (c) {
    ...
    case OPTION_MACH_EZ80_ADL: {
      const char *cpu_type = (c == OPTION_MACH_Z80) ? "z80" :
                             (c == OPTION_MACH_R800) ? "r800" :
                             (c == OPTION_MACH_Z180) ? "z180" :
                             (c == OPTION_MACH_EZ80_Z80) ? "ez80" : "ez80+adl";
      setup_march(cpu_type, &ins_ok, &ins_err, &cpu_mode);
      break;
    }
    ...
    case OPTION_MACH_IUP: {
      if ((ins_ok & INS_GBZ80) == 0) {
        if (c == OPTION_MACH_WUD || c == OPTION_MACH_IUD) {
          ins_ok |= INS_UNDOC;
          ins_err &= ~INS_UNDOC;
        } else {
...
void md_show_usage(FILE *f) {
    ...
    fprintf(f, "And EXT is combination (+EXT - add, -EXT - remove) of:\n");
        ...
        if (k + 1 < BUFLEN) {
            for (j = (1 << k); j > 0; --j) {
                for (unsigned int m = 0; m < k; ++m) {
                    buf[m] = (j & (1 << m)) ? TOUPPER(regtable[i].name[m]) : regtable[i].name[m];
                }
                buf[k] = '\0';
...
void z80_md_finish(void) {
    ...
    if (!bfd_set_arch_mach(stdoutput, TARGET_ARCH, mach_type)) {
...
int z80_start_line_hook(void) {
    char *p, *line_start, *rest, *name, *dollar;
    int len;
    char quote, buf[4];
...
symbolS *md_undefined_symbol(const char *name) {
  (void)name; // explicitly indicate the unused parameter
  return NULL;
}

const char *md_atof(int type, char *litP, int *sizeP) {
    if ((type == 'f' || type == 'F' || type == 's' || type == 'S') && str_to_float) {
        return str_to_float(litP, sizeP);
    }
    if ((type == 'd' || type == 'D' || type == 'r' || type == 'R') && str_to_double) {
...
long md_pcrel_from(fixS *fixp) {
    if (fixp == NULL || fixp->fx_frag == NULL) {
        return -1; // Return a default error value or handle error appropriately
    }
    return (long)fixp->fx_where + (long)fixp->fx_frag->fr_address;
}
...
static bool is_ins_type_valid(int ins_type) {
    return (ins_type & ins_ok) != 0;
}
    ...
    if (indir && (ins_ok & INS_GBZ80)) {
        ...
        if (!strncasecmp(p, "hl", 2)) {
            ...
            if (*skip_space(p + 1) == ')' && (*p == '+' || *p == '-')) {
                ...
                input_line_pointer = skip_space(p + 1) + 1;
...
int unify_indexed(expressionS *op) {
...
static const char *parse_expression(const char *s, expressionS *op) {
    const char *result = parse_exp_not_indexed(s, op);

    if (op->X_md && unify_indexed(op)) {
    ...
    if (op->X_op == O_constant && sdcc_compat && is_indir(result)) {
        expressionS offset = *op;
        result = parse_expression(result, op);
...
static const char *parse_cc(const char *s, char *op) {
    char buf[BUFLEN + 1];  // Ensure enough space for null terminator
    int i;
    struct reg_entry *cc_p = NULL;
    ...
    if ((i < BUFLEN) && (s[i] == '\0' || s[i] == ',')) {
        cc_p = bsearch(buf, cc_tab, ARRAY_SIZE(cc_tab), sizeof(cc_tab[0]), key_cmp);
    }
...
static const char *emit_insn(char prefix, char opcode, const char *args) {
    char *p = frag_more(prefix ? 2 : 1);
    if (p == NULL) {
        return NULL; // Handle error if frag_more fails
    }
    if (prefix) {
        *p++ = prefix;
    }
...
void z80_cons_fix_new(fragS *frag_p, int offset, int nbytes, expressionS *exp) {
    ...
    bfd_reloc_code_real_type relocations[] = {
        BFD_RELOC_8, BFD_RELOC_16, BFD_RELOC_24, BFD_RELOC_32
    };

    fix_new_exp(frag_p, offset, nbytes, exp, 0, relocations[nbytes - 1]);
}
...
static void emit_data_val(expressionS *val, int size) {
    ...
    switch (size) {
        ...
        default: as_fatal(_("invalid data size %d"), size);
    }

    if (val->X_op == O_register || val->X_op == O_md1 ||
        contains_register(val->X_add_symbol) || contains_register(val->X_op_symbol)) {
        ill_op();
    }
    ...
    if (size <= 2 && val->X_op_symbol) {
        ...
        if (val->X_op != O_right_shift) {
            shift = (val->X_op == O_bit_and) ? ((shift == (1 << (size * 8)) - 1) ? 0 : -1) : -1;
        }
    ...
    *p = val->X_add_number;
    ...
    if (val->X_op == O_constant) {
        if (val->X_add_number < -128 || val->X_add_number >= 128) {
            const char *msg = (r_type == BFD_RELOC_Z80_DISP8) ? _("index overflow (%+lld)") : _("offset overflow (%+lld)");
            as_bad(msg, (int64_t) val->X_add_number);
            return;
        }
    } else {
...
static void emit_word(expressionS *val) {
    int mode = (inst_mode & INST_MODE_IL) ? 3 : 2;
    emit_data_val(val, mode);
}
...
static const char *emit_m(char prefix, char opcode, const char *args) {
    expressionS arg_m;
    const char *p = parse_exp(args, &arg_m);
...
static const char *emit_s(char prefix, char opcode, const char *args) {
    ...
    bool is_generic_format = (*p == ',' && arg_s.X_md == 0 && arg_s.X_op == O_register && arg_s.X_add_number == REG_A);
    bool is_invalid_instruction = !(ins_ok & INS_EZ80) && !sdcc_compat;
    
    if (is_generic_format && is_invalid_instruction) {
        ill_op();
    }
...
static const char *emit_sub(char prefix, char opcode, const char *args) {
  if (!(ins_ok & INS_GBZ80)) {
...
static const char *emit_swap(char prefix, char opcode, const char *args) {
    expressionS reg;
    const char *p;
    char *q;
    ...
    bool is_invalid_reg = reg.X_md != 0 || reg.X_op != O_register || reg.X_add_number != REG_A;
    if (is_invalid_reg) {
        ill_op();
    }
...
static const char *emit_incdec(char prefix, char opcode, const char *args) {
    ...
    if (operand.X_op == O_register && (operand.X_md == 0) && (R_ARITH & operand.X_add_number)) {
...
emit_jr (char *args, char opcode)
{
    ...
    else
    {
        ...
        if (q == NULL)
        {
            handle_memory_error(); // Custom function to handle memory error
            return NULL;
        }
        *q = opcode;
        addr.X_add_number--; 
        if (!emit_byte(&addr, BFD_RELOC_8_PCREL))
        {
            handle_emit_error(); // Custom function to handle emit error
            return NULL;
        }
    }
    return p;
...
static const char * emit_jp(char prefix, char opcode, const char *args) {
    ...
    if (addr.X_md) {
        ...
        if (addr.X_op == O_register) {
            if (rnum == REG_HL || rnum == (REG_HL | R_INDEX)) {
...
static const char *emit_im(char prefix, char opcode, const char *args) {
    ...
    if (mode.X_md || mode.X_op != O_constant) {
        ill_op();
        return NULL; // Ensure function returns even after ill_op
    }

    if (mode.X_add_number < 0 || mode.X_add_number > 2) {
        ill_op();
        return NULL; // Ensure function returns even after ill_op
    }
    ...
    char *q = frag_more(2);
    *q++ = prefix;
    *q = opcode + 8 * (mode.X_add_number == 2 ? 3 : mode.X_add_number);
...
static const char *emit_pop(char prefix ATTRIBUTE_UNUSED, char opcode, const char *args) {
    expressionS regp;
    const char *p = parse_exp(args, &regp);

    bool is_valid_stackable_register = !regp.X_md && regp.X_op == O_register && (regp.X_add_number & R_STACKABLE);
    if (is_valid_stackable_register) {
        int rnum = regp.X_add_number;
        char *q;
...
{
    expressionS arg;
    const char *parsed_args = parse_exp(args, &arg);
    ...
    q = frag_more(2);
    fix_new_exp(frag_now, q - frag_now->fr_literal, 2, &arg, false, BFD_RELOC_Z80_16_BE);
...
static const char *emit_add(char prefix, char opcode, const char *args) {
    ...
    switch (term.X_add_number) {
        ...
        case REG_IX:
        case REG_IY: {
            int lhs = term.X_add_number;
            p = parse_exp(p, &term);
            int rhs = term.X_add_number;
            ...
            if ((term.X_op == O_register) && (rhs & R_ARITH) && (rhs == lhs || (rhs & ~R_INDEX) != REG_HL)) {
                q = frag_more((lhs & R_INDEX) ? 2 : 1);
                if (lhs & R_INDEX) *q++ = (lhs & R_IX) ? 0xDD : 0xFD;
                *q = opcode + ((rhs & 3) << 4);
                return p;
            }
            ...
            break; // Proceed to ill_op() by default
        }
...
static const char *emit_bit(char prefix, char opcode, const char *args) {
    ...
    bool isValidBitOp = !b.X_md && (b.X_op == O_constant) && (0 <= bn) && (bn < 8);
    if (isValidBitOp) {
        if (opcode == 0x40)
            return emit_m(prefix, opcode + (bn << 3), p); // Bit: no optional third operand
        else
            return emit_mr(prefix, opcode + (bn << 3), p); // Set, res: byte can be copied to register
    }
...
static const char *emit_bshft(char prefix, char opcode, const char *args) {
    expressionS r1, r2;
    ...
    if (!p || r1.X_md || r1.X_op != O_register || r1.X_add_number != REG_DE || 
        ...
        char opcode_in ATTRIBUTE_UNUSED, const char *args)
{
    ...
    if (op.X_op == O_register) {
        unsigned int reg_check = op.X_add_number | (op.X_md ? 0x8000 : 0);
...
static const char *emit_in(char prefix ATTRIBUTE_UNUSED, char opcode ATTRIBUTE_UNUSED, const char *args) {
    ...
    if (reg.X_md == 0 && reg.X_op == O_register && (reg.X_add_number <= 7 || reg.X_add_number == REG_F) && port.X_md) {
...
static const char *emit_in0(char prefix ATTRIBUTE_UNUSED, char opcode ATTRIBUTE_UNUSED, const char *args) {
    expressionS reg, port;
    const char *p = parse_exp(args, &reg);
    ...
    if (reg.X_md == 0 && reg.X_op == O_register && reg.X_add_number <= 7 &&
        port.X_md && port.X_op != O_md1 && port.X_op != O_register) {
        char *q = frag_more(2);
        q[0] = 0xED;
        q[1] = 0x00 | (reg.X_add_number << 3);
        emit_byte(&port, BFD_RELOC_8);
    } else {
        ill_op();
    }
          ...
          const char * args)
{
    ...
    if (reg.X_op == O_constant && reg.X_add_number == 0) {
        if (!(ins_ok & INS_Z80N)) check_mach(INS_OUT_C_0);
        reg.X_op = O_register;
        reg.X_add_number = 6;
    }
    ...
    if (port.X_op != O_register && port.X_op != O_md1) {
        ...
        if (REG_C == port.X_add_number || port.X_add_number == REG_BC) {
            if (port.X_add_number == REG_BC && !(ins_ok & INS_EZ80)) {
                ill_op();
                return p; // Additional error handling
            }
            char *q = frag_more(2);
            *q++ = 0xED;
...
emit_out0(char prefix ATTRIBUTE_UNUSED, char opcode ATTRIBUTE_UNUSED, const char *args) {
    ...
    bool valid_port = (port.X_md != 0 && port.X_op != O_register && port.X_op != O_md1);
    bool valid_reg = (reg.X_md == 0 && reg.X_op == O_register && reg.X_add_number <= 7);
...
static const char* emit_rst(char opcode, const char* args) {
    expressionS addr;
    const char* p = parse_exp_not_indexed(args, &addr);
...
static void emit_ld_m_n(expressionS *dst, expressionS *src) {
    char *q;
    ...
    if (dst->X_add_number == REG_HL) {
        prefix = 0x00;
    } else if (dst->X_add_number == REG_IX) {
        prefix = 0xDD;
...
static void emit_ld_m_r(expressionS *dst, expressionS *src) {
    ...
    if (dst->X_op == O_md1 && (ins_ok & INS_GBZ80)) {
        if (src->X_op == O_register && src->X_add_number == REG_A) {
            *frag_more(1) = (dst->X_add_number == REG_HL) ? 0x22 : 0x32;
            return;
        }
    } else if (dst->X_op == O_md1) {
        prefix = (dst->X_add_number == REG_IX) ? 0xDD : 0xFD;
    }

    if (dst->X_op == O_register) {
        if ((dst->X_add_number == REG_BC || dst->X_add_number == REG_DE) && src->X_add_number == REG_A) {
            q = frag_more(1);
            *q = 0x02 | ((dst->X_add_number & 3) << 4);
            return;
        }
        if ((dst->X_add_number == REG_IX || dst->X_add_number == REG_IY || dst->X_add_number == REG_HL) && src->X_add_number <= 7) {
            q = frag_more(prefix ? 2 : 1);
            if (prefix) {
                *q++ = prefix;
            }
...
static void emit_ld_m_rr(expressionS *dst, expressionS *src) {
    ...
    void handle_ez80_instructions(int dst_reg, int src_reg) {
        switch (dst_reg) {
    ...
    void handle_default_instructions(int src_reg) {
        switch (src_reg) {
    ...
    switch (dst->X_op) {
        ...
        case O_register:
            ...
            handle_ez80_instructions(dst->X_add_number, src->X_add_number);
            q = frag_more(prefix ? 2 : 1);
            if (prefix) *q++ = prefix;
            *q = opcode;
        ...
        default:
            if (ins_ok & INS_GBZ80) {
                ...
                handle_default_instructions(src->X_add_number);
            }
            q = frag_more(prefix ? 2 : 1);
            if (prefix) *q++ = prefix;
            *q = opcode;
...
static void emit_ld_r_m(expressionS *dst, expressionS *src) {
  ...
  if (dst->X_add_number == REG_A && src->X_op == O_register) {
    if (src->X_add_number == REG_BC || src->X_add_number == REG_DE) {
      opcode = (src->X_add_number == REG_BC) ? 0x0A : 0x1A;
      q = frag_more(1);
      *q = opcode;
      return;
    }
  ...
  if (src->X_op == O_md1 && (ins_ok & INS_GBZ80)) {
  ...
  if (src->X_op == O_register && dst->X_add_number <= 7) {
    prefix = (src->X_add_number == REG_IX) ? 0xDD : 
             (src->X_add_number == REG_IY) ? 0xFD : 
             (src->X_add_number == REG_HL) ? 0x00 : 
             (ill_op(), 0x00); // ill_op() returns void, for control flow
  ...
  if (src->X_op != O_register && dst->X_add_number == REG_A) {
    q = frag_more(1);
...
static void emit_ld_r_n(expressionS *dst, expressionS *src) {
    char *q;
    char prefix = 0;

    // Determine prefix based on the destination register
    ...
    q = frag_more(prefix ? 2 : 1);
    if (q == NULL) {
        exit(EXIT_FAILURE);  // Defensive programming: ensure memory allocation was successful
    }
...
static void emit_ld_r_r(expressionS *dst, expressionS *src) {
    ...
    if (opcode == 0) {
        switch (src->X_add_number) {
            case REG_A: case REG_B: case REG_C:
            case REG_D: case REG_E:
                break;
...
static void emit_ld_rr_m(expressionS *dst, expressionS *src) {
    char *q;
    int prefix = 0xED;
    int opcode;
    expressionS src_offset;
...
static void emit_ld_rr_nn(expressionS *dst, expressionS *src) {
    ...
    if (!q) {
        /* Add error handling if frag_more returns NULL */
        return;
    }
...
static const char * emit_lddldi(char prefix, char opcode, const char *args) {
    expressionS dst, src;
    const char *p;
    ...
    opcode = ((opcode & 0x08) ? 0x02 : 0x00) + 0x22;

    if ((dst.X_md != 0 && dst.X_add_number == REG_HL && src.X_md == 0 && src.X_add_number == REG_A) ||
        (dst.X_md == 0 && dst.X_add_number == REG_A && src.X_md != 0 && src.X_add_number == REG_HL)) {
        opcode |= (dst.X_add_number == REG_A) ? 0x08 : 0x00;
    } else {
        ill_op();
    }
...
emit_ldh (char prefix ATTRIBUTE_UNUSED, char opcode ATTRIBUTE_UNUSED, const char *args)
{
  ...
  int dstIsRegA = (dst.X_md == 0 && dst.X_op == O_register && dst.X_add_number == REG_A);
  int srcIsRegA = (src.X_md == 0 && src.X_op == O_register && src.X_add_number == REG_A);
  int srcIsNotMd1 = (src.X_md != 0 && src.X_op != O_md1);
  int dstIsNotMd1 = (dst.X_md != 0 && dst.X_op != O_md1);

  if (dstIsRegA && srcIsNotMd1) {
    if (src.X_op != O_register) {
      *frag_more(1) = 0xF0;
      emit_byte(&src, BFD_RELOC_8);
  ...
  } else if (dstIsNotMd1 && srcIsRegA) {
    if (dst.X_op == O_register && dst.X_add_number == REG_C) {
      *frag_more(1) = 0xE2;
    } else if (dst.X_op != O_register) {
      *frag_more(1) = 0xE0;
...
emit_ldhl (char prefix ATTRIBUTE_UNUSED, char opcode, const char *args)
{
    ...
    if (!is_valid_dst(&dst) || !is_valid_src(&src))
    {
        ill_op();
    }
...
static bool is_valid_dst(const expressionS *dst)
{
    return !(dst->X_md || dst->X_op != O_register || dst->X_add_number != REG_SP);
}

static bool is_valid_src(const expressionS *src)
{
    return !(src->X_md || src->X_op == O_register || src->X_op == O_md1);
}

static const char *parse_lea_pea_args(const char *args, expressionS *op) {
    const char *p = parse_exp(args, op);
    if (sdcc_compat && *p == ',' && op->X_op == O_register) {
        expressionS off;
        p = parse_exp(p + 1, &off);
        if (off.X_op != O_absent) {
            op->X_op = O_add;
            symbolS *symbol = make_expr_symbol(&off);
            if (symbol != NULL) {
                op->X_add_symbol = symbol;
            }
        }
    }
    return p;
...
static const char * emit_mlt(char prefix, char opcode, const char *args) {
    expressionS arg;
    ...
    char *q = frag_more(2);
    if (q == NULL) {
        return NULL; // Ensure q is not NULL to avoid errors
    }
...
/* MUL D,E (Z80N only) */
static const char *emit_mul(char prefix, char opcode, const char *args) {
  expressionS r1, r2;
  const char *p = parse_exp(args, &r1);
...
emit_nextreg(char prefix, char opcode ATTRIBUTE_UNUSED, const char *args)
{
    ...
    if (rr.X_md != 0 || nn.X_md != 0 || rr.X_op == O_register || rr.X_op == O_md1 || nn.X_op == O_md1)
    {
        ill_op();
        return NULL;
    }
...
{
    expressionS arg;
    const char *p = parse_lea_pea_args(args, &arg);
...
static const char * emit_reti(char prefix, char opcode, const char *args) {
    bool ins_gbz80 = ins_ok & INS_GBZ80;
    char primaryOpcode = ins_gbz80 ? 0x00 : prefix;
    char secondaryOpcode = ins_gbz80 ? 0xD9 : opcode;

    return emit_insn(primaryOpcode, secondaryOpcode, args);
}
...
{
    expressionS arg_s;
    const char *cursor = parse_exp(args, &arg_s);

    if (*cursor == ',' && arg_s.X_md == 0 && arg_s.X_op == O_register && arg_s.X_add_number == REG_A)
    {
        if (!(ins_ok & INS_EZ80))
            return ill_op();
        cursor = parse_exp(++cursor, &arg_s);
    }
...
static const char *emit_insn_n(char prefix, char opcode, const char *args) {
    expressionS arg;
    const char *p = parse_exp(args, &arg);
    
    if (!p || arg.X_md || arg.X_op == O_register || arg.X_op == O_md1) {
        ill_op();
    } else {
        char *q = frag_more(2);
        if (q) {
...
static void assume(int arg ATTRIBUTE_UNUSED) {
    char *name;
    char c;
    int n;
...
emit_mulub(char prefix ATTRIBUTE_UNUSED, char opcode, const char *args)
{
    ...
    char reg = TOLOWER(*(++p));
    const char *spaces_skipped = skip_space(p + 1);
    ...
    switch (reg) {
        ...
        case 'e':
            check_mach(INS_R800);
            if (!spaces_skipped || !*spaces_skipped) {
                char *q = frag_more(2);
                if (q) {
                    *q++ = prefix;
                    *q = opcode + ((reg - 'b') << 3);
...
static const char *emit_muluw(char opcode, const char *args) {
    const char *p = skip_space(args);
...
static int assemble_suffix(const char **suffix) {
    ...
    *suffix = p;
    ...
    const char **found = (const char **)bsearch(
        sbuf, suffixes, sizeof(suffixes) / sizeof(suffixes[0]), 
        sizeof(suffixes[0]), (int(*)(const void *, const void *))strcmp
    );
    ...
    static const int modes[] = {0x5B, 0x49, 0x5B, 0x5B, 0x49, 0x52, 0x52, 0x40};
    
    int index = found - suffixes;
    int mode_value = modes[index];

    *frag_more(1) = mode_value;

    static const int inst_modes[] = {
        INST_MODE_FORCED | INST_MODE_L | INST_MODE_IL,
        INST_MODE_FORCED | INST_MODE_L | INST_MODE_IS,
        INST_MODE_FORCED | INST_MODE_L | INST_MODE_IL,
        INST_MODE_FORCED | INST_MODE_L | INST_MODE_IL,
        INST_MODE_FORCED | INST_MODE_L | INST_MODE_IS,
        INST_MODE_FORCED | INST_MODE_S | INST_MODE_IL,
        INST_MODE_FORCED | INST_MODE_S | INST_MODE_IL,
        INST_MODE_FORCED | INST_MODE_S | INST_MODE_IS
    };

    inst_mode = inst_modes[index];
...
static void ignore(int arg) {
    (void)arg;  // Explicitly indicate the parameter is unused
    ignore_rest_of_line();
}

static void area(int arg) {
    if (!sdcc_compat) {
        as_fatal(_("Invalid directive"));
        return;
    }
    ...
    if (*p == '(') {
        *p = '\n';
        psect(arg);
        *p = '(';  // Restore character after function call
        ignore_rest_of_line();
    } else {
        psect(arg);
    }
...
int key_cmp(const void* a, const void* b) {
  return strcmp(*(const char**)a, ((const table_t*)b)->fp(0, 0, ""));
}

char* dwarf2_emit_insn(int x) {
  // Placeholder
  return NULL;
}

int assemble_suffix(const char** p) {
  // Placeholder
  return 1; 
}
...
void md_assemble(char* str) {
  ...
  char* old_ptr = input_line_pointer;
  
  while (i < BUFLEN && (isalnum(*p))) {
    buf[i++] = tolower(*p++);
  }

  if (i == BUFLEN) {
    memcpy(buf + BUFLEN - 3, "...", 3);
    buf[BUFLEN - 1] = '\0';
    as_bad("Unknown instruction '%s'", buf);
  ...
  if (!insp || (insp->inss && !(insp->inss & ins_ok))) {
    *frag_more(1) = 0;
    ...
    p = skip_space(p);
    if (!err_flag && *p) {
      as_bad("junk at end of line, first unrecognized character is '%c'", *p);
    }
  }
...
static int signed_overflow(signed long value, unsigned bitsize) {
    if (bitsize == 0 || bitsize > sizeof(signed long) * CHAR_BIT) {
        return 1;
    }
...
static int is_overflow(long value, unsigned bitsize) {
    if (bitsize >= sizeof(long) * CHAR_BIT) return 0;

    long max_value = (value < 0) ? -(1L << (bitsize - 1)) : (1L << bitsize) - 1;
    long min_value = (value < 0) ? -max_value - 1 : 0;
        ...
        switch (fixP->fx_r_type) {
            ...
            case BFD_RELOC_Z80_DISP8:
                if (signed_overflow(val, 8))
                    as_bad_where(fixP->fx_file, fixP->fx_line, _("8-bit signed offset out of range (%+ld)"), val);
                break;
            case BFD_RELOC_8:
                if (is_overflow(val, 8))
                    as_warn_where(fixP->fx_file, fixP->fx_line, _("8-bit overflow (%+ld)"), val);
                break;
            case BFD_RELOC_16:
                if (is_overflow(val, 16))
                    as_warn_where(fixP->fx_file, fixP->fx_line, _("16-bit overflow (%+ld)"), val);
                break;
            case BFD_RELOC_24:
                if (is_overflow(val, 24))
                    as_warn_where(fixP->fx_file, fixP->fx_line, _("24-bit overflow (%+ld)"), val);
                break;
            case BFD_RELOC_32:
                if (is_overflow(val, 32))
                    as_warn_where(fixP->fx_file, fixP->fx_line, _("32-bit overflow (%+ld)"), val);
                break;
            default:
                overflow = 1;
                break;
...
arelent *tc_gen_reloc(asection *seg ATTRIBUTE_UNUSED, fixS *fixp) {
    ...
    *reloc->sym_ptr_ptr = symbol_get_bfdsym(fixp->fx_addsy);
    reloc->address = (fixp->fx_r_type == BFD_RELOC_VTABLE_INHERIT || fixp->fx_r_type == BFD_RELOC_VTABLE_ENTRY)
                     ? fixp->fx_offset
...
int z80_tc_label_is_local(const char *name) {
    if (local_label_prefix == NULL || name == NULL) 
        return 0;
    for (const char *p = local_label_prefix; *p; p++, name++) {
        if (*name != *p) 
            return 0;
    }
    return 1;
...
#define EXP_MAX 1024  // Define appropriately
#define EXP_MIN -1023 // Define appropriately

static inline char *skip_space(const char *str) {
...
static int parse_exponent(char **p) {
    int exponent_sign = (**p == '-');
    if (exponent_sign || **p == '+') (*p)++;
    int exponent = 0;
    while (isdigit((unsigned char)**p)) {
        if (exponent < 100) exponent = exponent * 10 + (*(*p)++ - '0');
    }
    return exponent_sign ? -exponent : exponent;
}

static int str_to_broken_float(bool *signP, uint64_t *mantissaP, int *expP) {
    char *p = skip_space(input_line_pointer);
    *signP = (*p == '-');
    if (*signP || *p == '+') p++;
...
static const char *str_to_zeda32(char *litP, int *sizeP) {
    uint64_t mantissa;
    bool sign;
    int exponent;
    unsigned i;
    ...
    *sizeP = 4;
    ...
    // Adjust exponent and perform right shifts for mantissa
    exponent--;
    mantissa = (mantissa >> 39) + 1;  // Include rounding by incrementing

    // Ensure mantissa is within 24 bits and handle overflows
    if (mantissa >> 24) {
        mantissa >>= 1;
        exponent++;
    }

    // Handle exponent and mantissa for special cases
    if (exponent < -127 || mantissa == 0) {
        exponent = -128;
        mantissa = (exponent < -127 || mantissa == 0) ? 0x200000 : 0;
...
static const char* str_to_float48(char *litP, int *sizeP) {
    uint64_t mantissa;
    bool sign;
    int exponent;
    ...
    *litP++ = (char)(0x80 + exponent);
    for (unsigned i = 0; i < 40; i += 8) {
        *litP++ = (char)(mantissa >> i);
    }
    return NULL;
}

static const char *str_to_ieee754_h(const char *litP, int *sizeP) {
...
static const char *str_to_ieee754_s(const char *litP, int *sizeP) {
    if (litP == NULL || sizeP == NULL) {
        return NULL;
    }
...
static const char *str_to_ieee754_d(const char *litP, int *sizeP) {
    if (!litP || !sizeP) {
        return NULL;
    }
